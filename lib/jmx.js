...
class JMX {
  constructor(jmxFile) {
    ...
    // remove the dummy sampler from outputJS
    this.tgHT = _.get(
      this.outputJS,
  ...
  _createHeader(name, value) {
    ...
    const header = queryHeader.value;

    const headerClone = _.cloneDeep(header);

    // modify the object
    _.set(headerClone, "attributes.name", name);
    _.set(headerClone, "elements[0].elements[0].text", name);
    _.set(headerClone, "elements[1].elements[0].text", value);
  ...
  addNewHTTPSampler({ request, response, meta }) {
    const tmpHeaders = [];
    ...
    for (const key in headers) {
      if (headers.hasOwnProperty(key)) {
        ...
        if (
          ...
          const header = this._createHeader(key, headers[key]);
          if (header) {
